...
frappe.ui.form.on('Contract Worker Payroll Entry', {
    ...
    // ------------------- NEW MODULE DOC AUTO-FETCH -------------------
    module_doc: function(frm) {
        if (!frm.doc.module_doc) return;

        // Clear existing dependent fields first
        frm.events.clear_employee_table(frm);

        // Fetch values from selected module doc
        frappe.db.get_doc("Your Module DocType", frm.doc.module_doc).then(doc => {
            // List all fields you want to fetch
            frm.set_value("field1", doc.field1);
            frm.set_value("field2", doc.field2);
            frm.set_value("field3", doc.field3);
            frm.set_value("field4", doc.field4);
            frm.set_value("field5", doc.field5);
